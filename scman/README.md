...
PS5 Smart Cooler - отдельное устройство на базе **ESP32-C3 SuperMini** для мониторинга параметров PlayStation 5. Полная версия также позволяет управлять вентилятором и напряжениями APU - основного процессора, объединяющего вычислительную и графическую части консоли.

Устройство не изменяет операционную систему или программное обеспечение PS5. ESP32-C3 работает самостоятельно и остаётся доступной через веб-интерфейс, пока получает питание, даже если консоль выключена или не смогла загрузиться.

Бесплатная версия предназначена для чтения доступных показателей и настройки RGB-подсветки. Полная версия дополнительно поддерживает режимы управления вентилятором, переключение между управлением от Smart Cooler и PS5, настройку напряжений и прямой доступ к PMBus - протоколу обмена данными с контроллером питания.

Установка внутри консоли требует разборки, точной пайки и понимания электрической схемы. Перед началом работы прочитайте [предупреждения](#safety) и выберите подходящий [вариант установки](#installation-options).
...
<a id="warning"></a><a id="safety"></a>

## 1. Важные предупреждения

<a id="warning-install"></a><a id="installation-safety"></a>
...
<a id="warning-firmware"></a><a id="firmware-safety"></a>
...
В режиме **MANUAL** пользователь может установить скорость от 0 до 100%. Автоматической защиты от нулевой скорости в этом режиме нет: при значении 0% вентилятор может полностью остановиться.

Для штатной и рекомендуемой установки с управлением вентилятором используйте [дополнительную плату](#additional-board). Без неё режимы управления технически работают, но недоступно переключение SC/PS5 и отсутствует аварийный возврат к управлению от консоли. При отказе ESP32-C3 вентилятор может остановиться.

<a id="voltage-safety"></a>

### 1.4 Настройка напряжений

Неправильные значения напряжений могут вызвать нестабильную работу, невозможность запуска или повреждение APU и других компонентов PS5. Изменяйте значения небольшими шагами и сохраняйте их только после проверки стабильности.

Андервольт не является гарантированным способом исправления ошибки `CE-108255-1`: на одних консолях он уменьшает частоту сбоев, а на других не помогает.

<a id="01"></a><a id="firmware"></a>

## 2. Установка и обновление прошивки

<a id="01-01"></a><a id="firmware-requirements"></a>
...
Перед запуском отключите другие устройства, создающие виртуальные COM-порты. Убедитесь, что после подключения ESP32-C3 в диспетчере устройств Windows появляется новый COM-порт. Если потребуется, разрешите установщику доступ в интернет в брандмауэре.

<a id="01-02"></a><a id="installer-selection"></a>
...
<a id="01-03"></a><a id="first-install"></a>
...
При каждом обновлении память настроек перезаписывается. Сброс пользовательских параметров после обновления - нормальное поведение.

<a id="02"></a><a id="installation"></a>
...
| Гарантийные пломбы | Не повреждаются | Зависит от ревизии и способа доступа |
...
При полной установке подключаются SDA, SCL, питание и цепи вентилятора. На всех актуальных ревизиях достаточно снять верхнюю охлаждающую пластину и заменить термопрокладки с одной стороны. Жидкий металл при этом не затрагивается.

<a id="02-01"></a><a id="pinout"></a>
...
<a id="04"></a><a id="04-01"></a><a id="additional-board"></a>
...
Для рекомендуемой установки с управлением вентилятором дополнительная плата обязательна. Без неё управление технически возможно, но режим PS5 и аварийный возврат недоступны. При отказе ESP32-C3 вентилятор может остановиться.
...
- **Перемычка на резисторе 1 кОм в цепи PWM IN** - постоянное решение, если конкретной модели вентилятора недостаточно уровня управляющего сигнала.

Устанавливайте перемычки и изменяйте соединения только на полностью обесточенной консоли.

<a id="02-02"></a><a id="04-03"></a><a id="ps5-connection"></a>
...
5. Подключите сторону сигнала со стороны PS5 к контакту **IN** дополнительной платы.
...
<a id="04-04"></a><a id="pre-power-check"></a>
...
Модификация требует пайки непосредственно на ESP32-C3. Выполняйте её только на обесточенной плате.

<a id="02-03"></a><a id="wifi"></a>
...
<a id="03-06-02"></a><a id="wifi-router"></a>
...
6. Введите пароль сети и повторите его в **Confirm Password**.
...
Кнопка **RESET WI-FI TO DEFAULT** возвращает режим точки доступа с именем `PS5_Smart_Cooler` без пароля.
...
<a id="02-04"></a><a id="03"></a><a id="03-01"></a><a id="main-screen"></a>
...
<a id="03-02"></a><a id="temperature-ring"></a>
...
Анимация служит только для отображения и не влияет на управление охлаждением.

<a id="03-05"></a><a id="statistics"></a>
...
<a id="03-06"></a><a id="system-controls"></a>
...
<a id="03-06-03"></a><a id="i2c-status"></a>
...
Нажатие на значок включает или отключает игнорирование ошибки. Этот режим позволяет использовать управление без температуры, например OFFSET при неполной установке. На главном экране вместо температуры отображается `--°C`.

Игнорирование I2C не устраняет неисправность. Если температура должна быть доступна, проверьте SDA, SCL и [раздел устранения проблем](#troubleshooting-i2c).

<a id="03-06-05"></a><a id="sc-ps5"></a>
...
<a id="03-08"></a><a id="interface-v2"></a>
...
<a id="03-03-01"></a><a id="manual-mode"></a>
...
- значение 0% может полностью остановить вентилятор.

<a id="03-03-02"></a><a id="offset-mode"></a>
...
<a id="03-03-03"></a><a id="target-mode"></a>
...
<a id="03-03-04"></a><a id="graph-mode"></a>
...
Игнорирование ошибки I2C не заменяет данные температуры и не позволяет GRAPH работать по кривой. При потере связи восстановите I2C или временно выберите MANUAL либо OFFSET.

<a id="03-04"></a><a id="graph-editor"></a>
...
<a id="03-06-04"></a><a id="advanced-settings"></a><a id="monitoring"></a>
...
<a id="03-06-07"></a><a id="advanced-monitoring"></a>

### 7.2 Расширенный мониторинг

![Расширенный мониторинг](media/am_mode.png)

Расширенный мониторинг доступен в бесплатной и полной версиях. Он считывает данные по PMBus через подключение I2C и отображает:
...
Данные обновляются раз в секунду, когда PS5 включена и после её запуска прошло не менее 5 секунд. Температура VRAM через используемые интерфейсы недоступна.

<a id="03-06-09"></a><a id="pwm-diagnostics"></a>

### 7.3 Диагностика уровня PWM

Диагностика уровня PWM доступна только в полной версии.

Для работы диагностики PWM OUT дополнительной платы должен быть соединён с GPIO2 ESP32-C3. Это соединение необязательно для обычного управления вентилятором.
...
5. Если конкретному вентилятору недостаточно уровня сигнала, установите постоянную перемычку на резистор 1 кОм в цепи PWM IN. Делайте это только на полностью обесточенной консоли.

<a id="03-06-08"></a><a id="voltage-control"></a>

## 8. Настройка напряжений

Voltage Control и PMBus Direct Access доступны только в полной версии.

> **Внимание:** неправильные значения могут вызвать нестабильность, невозможность запуска или повреждение PS5. Перед изменениями запишите заводские значения. Не изменяйте напряжение более чем на ±5–10% от заводского без полного понимания последствий.
...
Для многих консолей подходят значения **VCORE 1,050 В** и **VGFX 0,950 В**, но это не универсальная гарантия стабильности. Используйте их только как отправную точку и обязательно проверяйте конкретную консоль
...
1. Дойдите до расположенной в начале игры сцены с аквариумом в полу.
2. Установите ракурс камеры по эталонному изображению.
3. Оставьте сцену на 5–10 минут.
...
На PS5 Slim сначала задайте `VOUT_MAX`, равный или больший требуемого напряжения. Затем применяйте `VOUT_COMMAND` и остальные регистры. Если `VOUT_MAX` ниже выбранного значения, контроллер питания может проигнорировать запись.
...
Ошибочная запись в произвольный регистр может вывести консоль из строя. Не используйте этот интерфейс для обычного подбора андервольта.

В нижней части панели отображаются сохранённые значения VID обеих страниц.

<a id="03-06-06"></a><a id="03-07"></a><a id="rgb"></a>
...
Интерфейс поддерживает от 1 до 20 светодиодов. Небольшой модуль можно питать от общей линии 5 В. Если суммарный ток может превысить 500 мА, используйте отдельный источник питания 5 В.

Не подключайте мощную ленту к линии 5 В ESP32-C3 без проверки допустимого тока источника.
...
- следующая температура не может быть ниже предыдущей;
...
Проверьте появление COM-порта, драйвер и USB-кабель. Закройте программы, которые могли занять этот порт, после чего полностью перезапустите установщик.

### 11.3 Установщик сообщает, что сервер недоступен

Проверьте интернет, настройки брандмауэра, VPN и прокси. Затем полностью перезапустите установщик. Состояние сервера и актуальные файлы проверяйте в группе поддержки.
...
3. Перезапустите устройство отключением питания на 10 секунд.
4. Проверьте, не подключилась ли ESP32-C3 к ранее сохранённому роутеру.
5. Поднесите телефон ближе к устройству и проверьте качество антенны.
...
5. Убедитесь, что логический уровень SDA/SCL составляет 3,3 В и на линиях установлены необходимые подтягивающие резисторы.

Если температура не нужна при неполной установке, можно включить [игнорирование I2C](#i2c-status). Это не ремонтирует соединение и не возвращает мониторинг.

### 11.13 ESP32-C3 не загружается после установки дополнительной платы

Полностью обесточьте устройство и проверьте отсутствие короткого замыкания между 5V, 3V и GND, а также ориентацию плат. Затем отсоедините ESP32-C3 от дополнительной платы и проверьте её запуск отдельно. Если отдельно плата работает, неисправность находится в монтаже или компонентах дополнительной платы.
...
Не проверяйте вентилятор подключением и отключением разъёма на работающей консоли.

### 11.15 В режиме PS5 отображается INPUT, но вентилятор не работает

Вероятная причина находится в дополнительной плате, мультиплексоре или цепи PWM. Проверьте GPIO5, правильность IN/OUT и качество пайки. Если вентилятор работает напрямую, но не работает через дополнительную плату, проверьте уровень сигнала и перемычку резистора 1 кОм.
...
<a id="05"></a><a id="faq"></a>

## 12. Часто задаваемые вопросы

### Чем отличаются бесплатная и полная версии?

Бесплатная версия читает доступные показатели и управляет RGB. Полная версия дополнительно управляет вентилятором, поддерживает SC/PS5, Voltage Control и PMBus Direct Access. Подробности отмечены в соответствующих разделах инструкции.
...
Поддерживаются все актуальные Fat, Slim и Pro. Расположение точек SDA/SCL различается и показано на фотографиях в разделе [Подключение к PS5](#ps5-connection).
...
Используйте установщик той же версии - Monitoring или Full. Перед подключением USB выключите PS5, отключите её от розетки и для безопасности отсоедините ESP32-C3 от остальной схемы. См. [полный порядок обновления](#firmware-update).
...
Device ID отображается в мониторе порта и в Advanced Settings. PIN - последние четыре цифры Device ID. Актуальная прошивка также выводит Device PIN в монитор порта.
...
<a id="03-10"></a><a id="04-06"></a><a id="support"></a>

## 13. Поддержка и полезные ссылки

- **Установщик полной версии:** [добавить ссылку](ССЫЛКА_НА_УСТАНОВЩИК_FULL)
- **Установщик бесплатной версии:** [добавить ссылку](ССЫЛКА_НА_УСТАНОВЩИК_MONITORING)
- **Группа поддержки PS5 Smart Cooler:** [добавить ссылку](ССЫЛКА_НА_ГРУППУ_ПОДДЕРЖКИ)
- **Видео установки на PS5 Fat:** [добавить ссылку](ССЫЛКА_НА_ВИДЕО_FAT)
- **Видео установки на PS5 Slim:** [добавить ссылку](ССЫЛКА_НА_ВИДЕО_SLIM)
- **Видео по Voltage Control:** [добавить ссылку](ССЫЛКА_НА_ВИДЕО_VOLTAGE_CONTROL)
- **Связь с разработчиком:** [добавить контакт](ССЫЛКА_НА_КОНТАКТ_РАЗРАБОТЧИКА)
